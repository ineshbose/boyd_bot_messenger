...
app.config["MSG"] = {
    ...
    "NEW_USER": lambda name, reg_id: (
        f"Hey there, {name}! I'm Boyd Bot - your university chatbot, here to make things easier. "
        f"To get started, register here: {app_url}/register/{reg_id}"
    ),
    
    "USER_IN_REG": lambda reg_id: (
...
app.config["LOG"] = {
    ...
    "FORMAT": "\033[94m[%(asctime)s] %(levelname)s in %(name)s:\033[96m %(message)s\033[0m",
    
    "INVALID_USER": lambda sender_id: f"{sender_id} is not a valid user",
    
    "USER_AUTH": lambda uid, login_result: f"{uid} undergoing registration. Result: {login_result}",
    
    "RELOGIN": lambda uid: f"{uid} logging in again.",
    
    "AUTH_FAIL": lambda uid, login_result: f"{uid} failed to log in. Result: {login_result}",
    
    "ERROR": lambda e_name, e_msg, uid, r_data: f"Exception ({e_name}) thrown: {e_msg}. {uid} requested '{r_data}'.",
